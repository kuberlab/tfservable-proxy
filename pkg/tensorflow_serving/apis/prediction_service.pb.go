...
func (c *predictionServiceClient) PredictJSON(ctx context.Context, in *ml_serving.PredictJSONData, opts ...grpc.CallOption) (*ml_serving.PredictJSONData, error) {
	out := new(ml_serving.PredictJSONData)
	err := grpc.Invoke(ctx, "/tensorflow.serving.PredictionService/PredictJSON", in, out, c.cc, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _PredictionService_PredictJSON_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ml_serving.PredictJSONData)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(PredictionServiceServer).PredictJSON(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/tensorflow.serving.PredictionService/PredictJSON",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(PredictionServiceServer).PredictJSON(ctx, req.(*ml_serving.PredictJSONData))
	}
	return interceptor(ctx, in, info, handler)
}
